...
PUBLIC int fs_readwrite(void)
{
  ...
  } else { /* writes to the meta data region of the file */
          scale = rip->i_sp->s_log_zone_size;
          /*allocate memory for the meta data region*/
        if (rip->i_zone[9] == NO_ZONE){
            /*allocates the memory*/
            rip->i_zone[9] = alloc_zone(rip->i_dev,rip->i_zone[9]);
            n = (block_t)rip->i_zone[9] << scale; /*gets the block number of the first block*/
            bp = get_block(rip->i_dev,n,NORMAL);
            zero_block(bp); /*setup the block for writing*/
        } else {
            n = (block_t)rip->i_zone[9] << scale;
            bp = get_block(rip->i_dev,n,NORMAL);
        }
          ...
          off = 0;
          cum_io = 0;
          block_size = get_block_size( (dev_t) rip->i_zone[9]);
          chunk = min(nrbytes, block_size - off);
          /*reads the message from the block and copies to the vfs*/
        if (rw_flag == READING) {
                if (bp->b_data == NULL){
                    fprintf(stderr,"NO data in meta region\n");
                } else {
                    r =    sys_safecopyto(VFS_PROC_NR, gid, (vir_bytes) cum_io,
                                    (vir_bytes) (bp->b_data+off), (size_t) chunk, D);
                }
        } else {
            /*writes the msg from the VFS to the block allocated*/
