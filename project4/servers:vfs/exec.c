...
/* Read the header and extract the text, data, bss and total sizes from it. */
  ...
  r = req_readwrite(vp->v_fs_e, vp->v_inode_nr, cvul64(pos), READING,
  		    VFS_PROC_NR, (char*)&hdr, sizeof(hdr), &new_pos, &cum_io, 0);
  if (r != OK) return r;
 ...
 */
  ...
  /* Issue request */
  r = req_readwrite(vp->v_fs_e, vp->v_inode_nr, cvul64(pos), READING,
  		    VFS_PROC_NR, buf, _MAX_BLOCK_SIZE, &new_pos, &cum_io, 0);
  if (r != OK) return(r);
 ...
 */
  ...
  if ((r = req_readwrite(vp->v_fs_e, vp->v_inode_nr, cvul64(off), READING,
  			 proc_e, 0, seg_bytes, &new_pos, &cum_io, 0)) != OK) {
