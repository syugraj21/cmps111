...
{
    return(meta_readwrite(READING));
}

/*===========================================================================*
 *				meta_readwrite			     *
 *===========================================================================*/
PUBLIC int meta_readwrite(rw_flag)
int rw_flag;			/* READING or WRITING */
{
    ...
    if (char_spec) {			/* Character special files. */
        ...
        /* Issue request
         * The last parameter for this function is used as flag to indicate a meta
         * read or write. Its set to 0 in all other calls to req_readwrite
         */
        r = req_readwrite(vp->v_fs_e, vp->v_inode_nr, position, rw_flag, who_e,
                          m_in.buffer, m_in.nbytes, &new_pos, &cum_io_incr, 1);
...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
  ...
  if (char_spec) {			/* Character special files. */
	...
	/* Issue request */
	r = req_readwrite(vp->v_fs_e, vp->v_inode_nr, position, rw_flag, who_e,
			  m_in.buffer, m_in.nbytes, &new_pos, &cum_io_incr, 0);
